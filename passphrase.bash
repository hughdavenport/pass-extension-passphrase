#!/usr/bin/env bash
# pass passphrase - Password Store Extension (https://www.passwordstore.org/)
...
cmd_passphrase_generate() {
	...
	[[ $err -ne 0 || ( $# -ne 2 && $# -ne 1 ) || ( $force -eq 1 && $inplace -eq 1 ) || ( $qrcode -eq 1 && $clip -eq 1 ) ]] && die "Usage: $PROGRAM $COMMAND generate [--clip,-c] [--qrcode,-q] [--in-place,-i | --force,-f] pass-name [number of words]"
	local path="$1"
	...
	pass=$(echo $(grep "^[a-z]\{3,7\}$" ${DICTIONARY} | shuf -n $length))
	[[ $(echo $pass | wc -w) -eq $length ]] || die "Could not generate password from /dev/urandom."
...
case "$1" in
	help|--help|-h) shift;	cmd_passphrase_usage "$@" ;;
	generate) shift;	cmd_passphrase_generate "$@" ;;
	*)			cmd_show "$@" ;;
esac
exit 0
